...
class IngestionPipeline:
    ...
    def process_packet(self, source_id: str, payload: Any) -> bool:
        ...
        # --- 2. TELEMETRY ---
        if hasattr(self.telemetry, 'record_ingestion'):
            self.telemetry.record_ingestion(source.id)

        # Format payload preview for debug/terminal
        preview = str(payload)
        if len(preview) > 60:
            preview = preview[:57] + "..."
        ...
        if source.status == SourceStatus.QUARANTINE:
            ...
            logger.info(f"🧪 [Pipeline: Quarentena] Sensor '{source.name}' ({source.id}) | Pacote VÁLIDO ({count}/{base_size}) | Amostra: {preview}")
        ...
        # B. ACTIVE PATH: Forward to System
            ...
            val_display = payload.get('value', payload) if isinstance(payload, dict) else payload
            logger.info(f"✅ [Pipeline: Ativo] Sensor '{source.name}' ({source.id}) | Pacote VÁLIDO | Valor: {val_display} | Payload: {preview}")
    ...
    def is_ready(self, source_id: str, consumer_tag: str = "linguist") -> bool:
        """Generic readiness check for consumers."""
    ...
    def is_ready_for_linguist(self, source_id: str) -> bool:
        ...
        return self.is_ready(source_id, consumer_tag="linguist")
